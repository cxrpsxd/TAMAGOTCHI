...
namespace TAMAGOTCHIW {
	...
	/// Сводка для DogGame
	/// </summary>
	public ref class DogGame : public System::Windows::Forms::Form
	{
	private:
		Dog* dog;
	...
	public:
		DogGame(String^ name, bool load)
		{
			if (load) {
				...
				dog = new Dog(Convert_String_to_string(name));
				dog->setHealth(min > 1000 ? std::max(0, pet->getHealth() - ((min - 1000) / 10)) : pet->getHealth());
				dog->setHunger(std::max(0, pet->getHunger() - min / 10));
				dog->setHappiness(std::max(0, pet->getHappiness() - min / 10));
				dog->setFatigue(std::max(0, pet->getFatigue() - min / 10));
				dog->setDirtiness(std::max(0, pet->getDirtiness() - min / 10));
				dog->setAge(pet->getAge());
				dog->setName(pet->getName());
				dog->setMoney(pet->getMoney());

				///////////////////////////
				dog->setPizza(pet->getPizza());
				dog->setBurger(pet->getBurger());
				dog->setCoce(pet->getCoce());
				...
				InitializeComponent();
				dog = new Dog(Convert_String_to_string(name));
				dog->setName(Convert_String_to_char(name));
				savePetToXML(dog);
				Name = name;
				UpdatePetStatus();
		...
		void UpdatePetStatus()
		{
			...
			String^ name1 = Convert_string_to_String(dog->getName());

			String^ health = Convert::ToString(dog->getHealth()) + "%";
			String^ hunger = Convert::ToString(dog->getHunger()) + "%";
			String^ happ = Convert::ToString(dog->getHappiness()) + "%";
			String^ fatig = Convert::ToString(dog->getFatigue()) + "%";
			String^ dirtiness = Convert::ToString(dog->getDirtiness()) + "%";
			String^ ag = Convert::ToString(dog->getAge()) + "%";
			String^ m = Convert::ToString(dog->getMoney());

			if (dog->getHunger() < 50) { timerFatigue->Start(); }
			else { timerFatigue->Stop(); }
			if (dog->getHunger() < 30) { timerHappy->Start(); }
			else { timerHappy->Stop(); }
			if (dog->getHunger() < 1) { timerHealth->Start(); }
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			System::ComponentModel::ComponentResourceManager^ resources = (gcnew System::ComponentModel::ComponentResourceManager(DogGame::typeid));
			...
			this->Hungry->Size = System::Drawing::Size(38, 17);
			...
			this->happiness->Size = System::Drawing::Size(42, 17);
			...
			this->fatigue->Size = System::Drawing::Size(38, 17);
			...
			this->dirty->Size = System::Drawing::Size(38, 17);
			...
			this->heal->Size = System::Drawing::Size(38, 17);
			...
			this->timerHunger->Tick += gcnew System::EventHandler(this, &DogGame::timerHunger_Tick);
			// 
			// timerFatigue
			// 
			this->timerFatigue->Interval = 8000;
			this->timerFatigue->Tick += gcnew System::EventHandler(this, &DogGame::timerFatigue_Tick);
			// 
			// timerHappy
			// 
			this->timerHappy->Interval = 5000;
			this->timerHappy->Tick += gcnew System::EventHandler(this, &DogGame::timerHappy_Tick);
			// 
			// timerHealth
			// 
			this->timerHealth->Interval = 2000;
			this->timerHealth->Tick += gcnew System::EventHandler(this, &DogGame::timerHealth_Tick);
			// 
			// timerAge
			// 
			this->timerAge->Enabled = true;
			this->timerAge->Interval = 300000;
			this->timerAge->Tick += gcnew System::EventHandler(this, &DogGame::timerAge_Tick);
			// 
			// gifs
			// 
			...
			this->gif->Tick += gcnew System::EventHandler(this, &DogGame::gif_Tick);
			// 
			// Feed1
			// 
			...
			this->Feed1->Click += gcnew System::EventHandler(this, &DogGame::Feed1_Click);
			// 
			// heal1
			// 
			...
			this->heal1->Click += gcnew System::EventHandler(this, &DogGame::heal1_Click);
			// 
			// play1
			// 
			...
			this->play1->Click += gcnew System::EventHandler(this, &DogGame::play1_Click);
			// 
			// wash1
			// 
			...
			this->wash1->Click += gcnew System::EventHandler(this, &DogGame::wash1_Click);
			// 
			// sleep1
			// 
			...
			this->sleep1->Click += gcnew System::EventHandler(this, &DogGame::sleep1_Click);
			// 
			// button_exit
			// 
			...
			this->button_exit->Click += gcnew System::EventHandler(this, &DogGame::button_exit_Click_1);
			// 
			// pictureBox1
			// 
			...
			this->money->Size = System::Drawing::Size(59, 20);
